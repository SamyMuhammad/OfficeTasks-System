    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="form-title">إضافة قسم</h4>
            </div>
            <div class="modal-body">
                <form id="create-form-element" action="{{ route('admin.categories.store') }}" method="POST">
                    @csrf
...
<script>
    function editCategory(itemId) {
        var update_route = "{{ route('admin.categories.update', ':id') }}";
        update_route = update_route.replace(':id', itemId);
        $('#edit-form-element').attr('action', update_route);

        var edit_route = "{{ route('admin.categories.edit', ':id') }}";

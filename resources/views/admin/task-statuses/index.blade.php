...
@section('title', 'حالات المهمات')
@include('layouts._datatables_assets')

@section('extra-css')
@endsection
@section('bar-title', 'حالات المهمات')
@section('page-title')
حالات المهمات
@can('create task-statuses')
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if ($item->isEditable())
                                        ...
                                        <button onclick="editTaskStatus('{{ $item->id }}')" class="btn purple-sharp btn-outline sbold uppercase">تعديل</button>
                                        @endcan

                                        @can('delete task-statuses')
                                        <form action="{{ route('admin.task-statuses.destroy', $item->id) }}" method="POST" class="destroy-form">
                                            @method('DELETE')
                                            @csrf
...
@include('admin.task-statuses._modal')
@endsection
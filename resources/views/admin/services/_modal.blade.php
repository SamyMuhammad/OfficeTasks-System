    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="create-form-element" action="{{ route('admin.services.store') }}" method="POST">
                    @csrf
...
<script>
    function editCategory(itemId) {
        var update_route = "{{ route('admin.services.update', ':id') }}";
        update_route = update_route.replace(':id', itemId);
        $('#edit-form-element').attr('action', update_route);

        var edit_route = "{{ route('admin.services.edit', ':id') }}";

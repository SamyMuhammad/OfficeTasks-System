                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td>{{ optional($item->expense_type)->name }}</td>
                                <td>{{ $item->source }}</td>
                                <td>{{ $item->isPaidInText }}</td>
                                <td>{{ $item->created_at->format('Y-m-d') }}</td>
                                <td>
                                    {{-- @can('show expenses')
                                    <a href="{{ route('expenses.show', $item->id) }}" class="btn blue-sharp btn-outline sbold uppercase">عرض</a>
                                    @endcan --}}
                                    
                                    @can('edit expenses')
                                    <a href="{{ route('expenses.edit', $item->id) }}" class="btn purple-sharp btn-outline sbold uppercase">تعديل</a>
                                    @endcan

                                    @can('delete expenses')
                                    <form action="{{ route('expenses.destroy', $item->id) }}" method="POST" class="destroy-form">
                                        @method('DELETE')
                                        @csrf
